...
void savefile(Player p, int countinv, int seed, int size_map_width, int size_map_length, char** map, int equipedshild, int equipedsword, int place, int place_before, int isininv, int nbtask, int taskeffectued, int mobkilled, int lootpicked, int lenght_max_room, int width_max_room, int tot_room, int count, int tot_door, Room * room, int timer, int count_room, int nb_room){
    ...
    fprintf(save, "%d ", countinv);
    fprintf(save, "%d ", p.posy);
    fprintf(save, "%d ", p.posx);
    fprintf(save, "%d ", p.live);
    fprintf(save, "%d ", p.playerStat.tot_pv);
    fprintf(save, "%f ", p.playerStat.pv);
    fprintf(save, "%d ", p.playerStat.atck);
    fprintf(save, "%d ", p.playerStat.def);
    fprintf(save, "%d ", p.playerStat.level);
    fprintf(save, "%d ", p.playerStat.dodge);
    fprintf(save, "%f ", p.playerStat.exp);
    fprintf(save, "%d ", p.playerStat.speed);
    fprintf(save, "%d ", equipedshild);
    fprintf(save, "%d ", equipedsword);
    fprintf(save, "%d ", place);
    fprintf(save, "%d ", place_before);
    fprintf(save, "%d ", isininv);
    fprintf(save, "%d ", nbtask);
    fprintf(save, "%d ", taskeffectued);
    fprintf(save, "%d ", mobkilled);
    fprintf(save, "%d ", lootpicked);
    fprintf(save, "%d ", lenght_max_room);
    fprintf(save, "%d ", width_max_room);
    fprintf(save, "%d ", tot_room);
    fprintf(save, "%d ", tot_door);
    fprintf(save, "%d ", count);
    fprintf(save, "%d ", timer);
    fprintf(save, "%d ", count_room);
    fprintf(save, "%d ", nb_room);
    ...
    fprintf(save, "%d ", seed);
    fprintf(save, "%d ", size_map_width);
    fprintf(save, "%d ", size_map_length);
...
void startagame(WINDOW * win, int winposx, int winposy, int winlength, int winwidth, int maxroomwidth, int maxroomlength, int minroom, int isloaded, char * name ){
    ...
    if(isloaded != 1){
        seed = giveSeed(win, winwidth, winlength);
        srand(seed);
    }
    ...
    if(isloaded == 1){
        ...
        j.name = malloc((sizeof(name)+1)*sizeof(char));
    ...
    if(isloaded == 1){
        ...
        for(int i = 0; i < countroom ; i++){
            ...
            room[loadroom].event = NULL;

            room[loadroom].event= calloc(room[loadroom].nbevent, sizeof(Event));

            if(room[loadroom].event == NULL){
                printf("Error with the event alloctation");
                exit(29);
            }
            ...
            room[loadroom].nbdoor = malloc(sizeof(Door)*4);

            if(room[loadroom].nbdoor == NULL){
                printf("Error with the Door allocation");
                exit(8);
            }
            ...
            room[loadroom].room = NULL;

            room[loadroom].room = malloc(sizeof(char*)*(room[loadroom].width + 2));

            if(room[loadroom].room == NULL){
                printf("Error with the room allocation \n");
                exit(27);
            }

            for (int i = 0; i<room[loadroom].width+2; i++){
                room[loadroom].room[i] = NULL;
                room[loadroom].room[i] = malloc(sizeof(char)*(room[loadroom].length + 2));
                if (room[loadroom].room[i] == NULL){
                    printf("Error with the room allocation \n");
                    exit(28);
                }
            }
            ...
            for (int j = 0; j < room[loadroom].width+2; ++j) {
                for (int p = 0; p < room[loadroom].length+2; ++p) {
                    fscanf(save, "%d", &temp);
                    room[loadroom].room[j][p] = temp;
                }
            }
        ...
        for(int i = 0; i<10; i++){
            ...
            if(j.inventory[i].type == 0){
                strcpy(j.inventory[i].name, "Pistol");
            }
            if(j.inventory[i].type == 1){
                strcpy(j.inventory[i].name, "Shield");
            }
            if(j.inventory[i].type == 2){
                strcpy(j.inventory[i].name, "Potion");
            }
            if(j.inventory[i].type == 3){
                strcpy(j.inventory[i].name, "Lightsaber");
            }
    ...
    if(isloaded != 1){
        ...
        for(int i = 0; i<10; i++){
            ...
            j.inventory[i].name[0] = 'N';
            j.inventory[i].name[1] = '\0';
